    ...
    async execute(interaction: ButtonInteraction): Promise<void> {
        ...
        if (!player) {
            await interaction.reply({
                content:
                    "Link your BeatLeader profile using /link to vote on scores!",
                ephemeral: true,
            });
            return;
        }
        const score = await Score.findOne({
            messages: { $elemMatch: { messageId: interaction.message.id } },
        });
        if (!score) return;
        ...
        switch (interaction.customId) {
            case "score-like": {
                if (score.downVoteIds.includes(player.beatLeaderId)) {
                    score.downVoteIds = score.downVoteIds.filter(
                        (id) => id != player.beatLeaderId
                    );
                }
                if (score.upVoteIds.includes(player.beatLeaderId)) {
                    score.upVoteIds = score.upVoteIds.filter(
                        (id) => id != player.beatLeaderId
                    );
                    await interaction.reply({
                ...
                if (channel && channel.isTextBased()) {
                    ...
                    if (message) {
                        ...
                        const dislike = new ButtonBuilder()
                            ...
                            .setLabel(
                                "👎 Dislike • " + score.downVoteIds.length
                            )
