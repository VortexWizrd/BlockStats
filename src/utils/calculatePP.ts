...
const accSaberScale = 61.0;
const accSaberShift = -18.0;
const accSaberCurve = [
...
const accSaberSlopes = getSlopes(accSaberCurve);
...
function getCurveMultiplier(curve: CurvePoint[], slopes: number[], acc: number): number {
...
export function getAccSaberAP(complexity: number, acc: number) {
    return getCurveMultiplier(accSaberCurve, accSaberSlopes, acc) * (complexity - accSaberShift) * accSaberScale;
}