...
module.exports = {
    data: new SlashCommandBuilder()
        ...
        .addSubcommand((cmd) =>
            cmd
                .setName("show")
                .setDescription("Display a BlockStats profile")
                .addStringOption((option) =>
                    option
                        .setName("id")
                        .setDescription("BeatLeader ID")
                        .setRequired(false),
                )
                .addStringOption((option) =>
                    option
                        .setName("name")
                        .setDescription("BeatLeader name")
                        .setRequired(false),
                ),
        ),
        ...
        switch (subCommand) {
            case "link": {
                ...
                const beatLeaderData: any =
                    await BeatLeaderAPI.getUserFromDiscord(interaction.user.id);
                ...
                if (player) {
                    if (
                        beatLeaderData &&
                        beatLeaderData.id === player.beatLeaderId
                    ) {
                        if (
                            scoreSaberId &&
                            ...
                            return await interaction.reply({
                                content:
                                    "Your BeatLeader account is already linked!",
                                ephemeral: true,
                            });
                        }
                    } else {
            ...
            case "show": {
                ...
                if (interaction.options.get("id")) {
                    player = await Player.findOne({
                        beatLeaderId: interaction.options.getString("id"),
                    });
                    if (!player) {
                ...
                const embed = new EmbedBuilder()
                    ...
                    .setDescription(
                        `${linkText}\n# #${player.blRankHistory[player.blRankHistory.length - 1].rank} <:beatleader:1492695343345832102> ${player.ssRankHistory ? `| #${player.ssRankHistory[player.ssRankHistory.length - 1].rank} <:scoresaber:1492695389634035823>` : ""}`,
                    )
                    .setColor("Default")
                    .addFields({
                        name: "Scores",
