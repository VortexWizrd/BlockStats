...
module.exports = {
        ...
        .addSubcommand((subCommand) =>
            subCommand
                ...
                .addUserOption((option) =>
                    option.setName("user").setDescription("Discord user")
                )
                .addIntegerOption((option) =>
                    option.setName("page").setDescription("Page number")
                )
    ...
    async execute(interaction: ChatInputCommandInteraction) {
        ...
        if (isGlobal) {
            ...
            if (!Player.findOne({ discordId: interaction.user.id }))
        ...
        switch (sort) {
            case "recent": {
                ...
                scores = scores.sort(
                    (a: any, b: any) =>
                        Number(b.beatLeaderData.timeset || 0) -
                        Number(a.beatLeaderData.timeset || 0)
                );
            ...
            case "likes": {
                ...
                scores = scores.sort(
                    (a: any, b: any) =>
                        Number(b.upVoteIds.length) - Number(a.upVoteIds.length)
                );
            ...
            case "dislike": {
                ...
                scores = scores.sort(
                    (a: any, b: any) =>
                        Number(b.downVoteIds.length) -
                        Number(a.downVoteIds.length)
                );
            ...
            case "highest-ratio": {
                ...
                scores = scores.sort((a: any, b: any) => {
                    const aTotalVotes =
                        a.upVoteIds.length + a.downVoteIds.length == 0
                            ? 1
                            : a.upVoteIds.length + a.downVoteIds.length;
                    const bTotalVotes =
                        b.upVoteIds.length + b.downVoteIds.length == 0
                            ? 1
                            : b.upVoteIds.length + b.downVoteIds.length;

                    const aRatio =
                        a.upVoteIds.length == a.downVoteIds.length
                            ? 0.5
                            : a.upVoteIds.length / aTotalVotes;
                    const bRatio =
                        b.upVoteIds.length == b.downVoteIds.length
                            ? 0.5
                            : b.upVoteIds.length / bTotalVotes;
            ...
            if (i < scores.length) {
                ...
                if (sort === "likes") {
                    ...
                    const ratio =
                        ...
                        scores[i].downVoteIds.length
                            ? 0.5
                            : scores[i].upVoteIds.length / totalVotes;

                    sortValue = (ratio * 100).toFixed() + "% rating";
                }

                sortList += `## ${i + 1}. ${
                    scores[i].beatLeaderData.leaderboard.song.name
                } | #${scores[i].beatLeaderData.rank} • ${(
                    scores[i].beatLeaderData.accuracy * 100
                ).toFixed(2)}%${sortValue === "" ? "" : " | " + sortValue}\n${
                    !isGlobal
                        ? ""
                        : `### Set by ${scores[i].beatLeaderData.player.name}\n`
                }[[Replay](https://replay.beatleader.com/?link=${
                    scores[i].beatLeaderData.replay
                })] [ID: ${scores[i]._id}]\n`;
            }
        }
        ...
        const embed = new EmbedBuilder()
            .setTitle(
                `${
                    isGlobal ? "" : user.displayName + "'s "
                }${sortTitle} Scores${page === 1 ? "" : ` (Page ${page})`}`
            )
