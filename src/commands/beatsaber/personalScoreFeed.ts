...
module.exports = {
    ...
    async execute(interaction: ChatInputCommandInteraction) {
        if (interaction.guild) {
            await interaction.reply({ content: 'You must DM the bot to use this command!', ephemeral: true });
            return;
        }
        ...
        switch (interaction.options.getSubcommand()) {
            case 'add': {    
                ...
                if (existingFeed) {
                    await interaction.reply({ content: 'You have already set up a personal score feed! Please use the `edit` or `remove` command to modify it.', ephemeral: true });
                    return;
                }
            ...
            case 'remove': {
                ...
                const feed = await ScoreFeed.findOne({ userId: interaction.user.id });
                if (!feed) {
                    await interaction.reply({ content: 'No score feed exists!', ephemeral: true });
                    return;
                }
